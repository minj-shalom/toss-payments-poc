...
export default class AxiosAPIAdapter {
  ...
  async getHeader(headerKey: string) {
    return this.session.defaults.headers.common[headerKey];
  }

  async setHeader(headerKey: string, headerValue: string) {
    return (this.session.defaults.headers.common[headerKey] = headerValue);
  }

  async resetHeader(headerKey: string) {
    return delete this.session.defaults.headers.common[headerKey];
  }
